...
# Carregar dados já unidos (versão estendida)
with open('data/meteo_vazao_joined_extended.json') as f:
...
def shift_flow_preciso(group):
    ...
    for i in range(len(group)):
        if i < len(group) - 1 and is_next_month(group.iloc[i], group.iloc[i+1]):
            ...
            flow_next_max.append(group.iloc[i+1]['flow_max'])
            flow_next_min.append(group.iloc[i+1]['flow_min'])
        else:
            flow_next.append(None)
            flow_next_max.append(None)
            flow_next_min.append(None)
    
    group['flow_next_month'] = flow_next
    group['flow_next_month_max'] = flow_next_max
    group['flow_next_month_min'] = flow_next_min
    return group
...
filtered = filtered.drop(columns=['flow', 'flow_max', 'flow_min', 'date'])

# Salvar resultado
with open('data/meteo_vazao_shifted_extended.json', 'w') as f:
    json.dump(filtered.to_dict('records'), f, indent=2)

print('Arquivo data/meteo_vazao_shifted_extended.json gerado com deslocamento temporal e filtragem estendida.')